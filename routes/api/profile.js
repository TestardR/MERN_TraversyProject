...
router.get(
  ...
  (req, res) => {
    ...
    Profile.findOne({ user: req.user.id })
      // Since we connected the user collection to the profile's, see profileSchema
...
// @route   GET api/profile/handle/:handle // Backend API route, will be hit by front-end but not used by user
...
router.post(
  ...
  (req, res) => {
    ...
    Profile.findOne({ user: req.user.id }).then(profile => {
      ...
      // Add to exp array // unshift adds a the beginning of the array
    ...
    Profile.findOne({ user: req.user.id })
      .then(profile => {
        ...
        const removeIndex = profile.education
          .map(item => item.id) // we get an array of experiences by id
          .indexOf(req.params.edu_id); // we get the correct experience within the array (exp_id)
        // Splice out of array (profile.experience is the array)
